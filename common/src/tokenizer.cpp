...
SPTokenizer::SPTokenizer(const std::string &model_path) {
    ...
    if (pad_id < 0 || eos_id < 0) {
        throw std::runtime_error("Tokenizer does not contain <pad> or </s> pieces.");
    }
}

std::vector<int> SPTokenizer::encode(const std::string &text, bool add_eos) {
    std::vector<int> ids;
    if (!sp.Encode(text, &ids)) {
...
std::string SPTokenizer::decode(const std::vector<int> &ids) {
    std::string out;
    if (!sp.Decode(ids, &out)) {
        throw std::runtime_error("SentencePiece decode failed");
    }
    return out;
}

int SPTokenizer::get_pad_id() const { return pad_id; }
int SPTokenizer::get_eos_id() const { return eos_id; }