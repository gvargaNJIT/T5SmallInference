...
// Serial version of matmul for use within attention heads
// to avoid nested MPI parallelization
Tensor serial_matmul(const Tensor& a, const Tensor& b)
{
    ...
    for (int row = 0; row < m; row++)
        for (int col = 0; col < p; col++)
        {
            float sum = 0.f;
            for (int i = 0; i < n; i++)
                sum += a.data[row * n + i] * b.data[i * p + col];
    ...
    float* output_ptr)
{
    ...
    Tensor scores = serial_matmul(q_h, k_h.transpose());
    ...
    Tensor head_out = serial_matmul(scores, v_h);
